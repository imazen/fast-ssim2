...
- **`ToLinearRgb for yuvxyb::Rgb` could abort the process on a valid image.** Its non-sRGB arm unwrapped `LinearRgb::try_from` with `.expect("Rgb to LinearRgb conversion should not fail")`. It does fail: H.273 TC=17 (`ST428`, digital cinema) — and TC=0/3 `Reserved0`/`Reserved` and TC=22 `BT1361E` — have no `to_linear` in yuvxyb, so an ST428-tagged image panicked instead of returning an error. Now `Err(Ssimulacra2Error::LinearRgbConversionFailed)`, with a regression test (68c22ee)
