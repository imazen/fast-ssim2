...
/// On x86_64: AVX2+FMA via f32x8. Falls back to scalar on unsupported CPUs.
use archmage::incant;
...
pub(crate) fn ssim_map_simd(
    ...
    incant!(ssim_map_inner(width, height, m1, m2, s11, s22, s12))
}
...
pub(crate) fn edge_diff_map_simd(
    ...
    incant!(edge_diff_map_inner(width, height, img1, mu1, img2, mu2))
}
...
pub(crate) fn image_multiply_simd(
    ...
    incant!(image_multiply_inner(img1, img2, out))
}
